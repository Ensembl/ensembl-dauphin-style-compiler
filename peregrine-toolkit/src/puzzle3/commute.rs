...
impl<'f,'a,T> Commuter<'f,'a,T> {
    fn new<F: 'a>(initial: T, compose: F) -> Commuter<'f,'a,T> where F: Fn(&T,&T) -> T + 'f {
        Commuter { initial: Arc::new(initial), compose: Box::new(compose), rest: vec![] }
    }
