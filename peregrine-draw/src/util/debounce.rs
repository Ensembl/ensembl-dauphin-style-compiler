...
async fn debounce_agent(needed: Weak<Mutex<Needed>>, period_ms: f64, cb: Arc<Box<dyn Fn() + 'static>>) -> Result<(),Message> {
    loop {
...
// XXX run level audit
impl Debounce {
    pub(crate) fn new<F>(commander: &PgCommanderWeb, period_ms: f64, cb: F) -> Debounce where F: Fn() + 'static {
        ...
        commander.add("debounce",6,None,None,Box::pin(debounce_agent(weak_needed,period_ms,callback)));
        Debounce(needed)
    }
