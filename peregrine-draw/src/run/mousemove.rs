...
fn mouse_move_tick(input: &Input, mouse_position: &mut Option<(f64,f64)>, stage: &ReadStage, train_set: &GlTrainSet) -> Result<(),Message> {
...
async fn mouse_move_loop(input: Input, train_set: GlTrainSet, stage: Arc<Mutex<Stage>>) {
