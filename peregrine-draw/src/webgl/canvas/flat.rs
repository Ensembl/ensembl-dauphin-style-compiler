...
impl Flat {
    pub(super) fn new(document: &Document, weave: &CanvasWeave, size: (u32,u32)) -> Result<Flat,Message> {
        let el = js_error(document.create_element("canvas"))?;
        let canvas_el = el.dyn_into::<HtmlCanvasElement>().map_err(|_| Message::XXXTmp("could not cast canvas to HtmlCanvasElement".to_string()))?;
        canvas_el.set_width(size.0);
        canvas_el.set_height(size.1);
        let context = canvas_el
            .get_context("2d").map_err(|_| Message::XXXTmp("cannot get 2d context".to_string()))?
            .unwrap()
            .dyn_into::<CanvasRenderingContext2d>().map_err(|_| Message::XXXTmp("cannot get 2d context".to_string()))?;
        Ok(Flat {
    ...
    pub(crate) fn set_font(&mut self, pen: &Pen) -> Result<(),Message> {
        if self.discarded { return Err(Message::XXXTmp(format!("discarded flat canvas"))); }
    ...
    pub(crate) fn measure(&mut self, text: &str) -> Result<(u32,u32),Message> {
        if self.discarded { return Err(Message::XXXTmp("discarded flat canvas".to_string())) }
        let width = js_error(self.context.as_ref().unwrap().measure_text(text))?.width();
        let height = self.font_height.ok_or_else(|| Message::XXXTmp("no font set before measure".to_string()))?;
    ...
    pub(crate) fn text(&self, text: &str, origin: (u32,u32), size: (u32,u32), colour: &DirectColour) -> Result<(),Message> {
        ...
        js_error(context.fill_text(text,origin.0 as f64,origin.1 as f64));
        Ok(())
    }
