...
use peregrine_data::{AllAllotmentsRequest, Assets, VariableValues};
...
impl SpectreManager {
    ...
    pub(crate) fn draw(&mut self, allotment_petitioner: &mut AllAllotmentsRequest, gl: &mut WebGlGlobal, assets: &Assets, stage: &ReadStage, session: &DrawingSession) -> Result<(),Message> {
