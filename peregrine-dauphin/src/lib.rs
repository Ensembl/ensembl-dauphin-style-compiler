...
fn run_eardo(interp: &mut Interpreter, data_store: &DataStore, libcore_builder: &LibcoreBuilder, libperegrine_builder: &LibPeregrineBuilder, commander: &PgCommander, spec: PgEardoRunTaskSpec, stream: CommanderStream<Result<(),Error>>) {
    /* run */
    let stream = stream.clone();
...
async fn main_loop(integration: Box<dyn PgDauphinIntegration>, core: PeregrineCore) -> Result<(),Error> {
    ...
    loop {
        ...
        match e.task {
            ...
            PgDauphinTaskSpec::RunEardo(r) => run_eardo(&mut interp,&data_store,&libcore_builder,&libperegrine_builder,&core.base.commander,r,e.channel),
