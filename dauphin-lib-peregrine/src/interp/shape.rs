...
impl InterpCommand for Rectangle2InterpCommand {
    fn execute(&self, context: &mut InterpContext) -> anyhow::Result<CommandResult> {
        ...
        console::log_1(&format!("rectangle2 x=({:?} {:?} {:?}) y=({:?} {:?} {:?})",sea_x,ship_x0,ship_x1,sea_y,ship_y0,ship_y1).into());
...
impl InterpCommand for Rectangle1InterpCommand {
    fn execute(&self, context: &mut InterpContext) -> anyhow::Result<CommandResult> {
        ...
        console::log_1(&format!("rectangle1 x=({:?} {:?} {:?}) y=({:?} {:?} {:?})",sea_x,ship_x,size_x,sea_y,ship_y,size_y).into());
        Ok(CommandResult::SyncResult())
    }
}
