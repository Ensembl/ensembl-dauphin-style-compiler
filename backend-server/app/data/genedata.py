...
from .numbers import delta, alternate, zigzag, lesqlite2, compress, classify
...
def extract_gene_data(chrom: Chromosome, panel: Panel) -> Response:
    ...
    out['starts'] = compress(lesqlite2(zigzag(alternate(delta(delta([ x[0] for x in gene_sizes ]))))))
    out['lengths'] = compress(lesqlite2(zigzag(alternate(delta([ x[1]-x[0] for x in gene_sizes ])))))
