...
use crate::{DataMessage, LaneStore, PeregrineCoreBase, PgCommanderTaskSpec, ShapeList, ShapeListBuilder, ShapeRequest, add_task, api::MessageSender};
...
pub(crate) async fn load_shapes(base: &PeregrineCoreBase, result_store: &LaneStore, messages: Option<&MessageSender>, shape_requests: Vec<ShapeRequest>, mode: &LoadMode) -> (Option<ShapeList>,Vec<DataMessage>) {
    let mut errors = vec![];
