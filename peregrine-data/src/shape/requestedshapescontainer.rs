...
pub struct RequestedShapesContainer {
    shapes: Vec<Shape<LeafRequest>>,
    carriage_universe: Arc<LeafRequestSource>,
    stats: OriginStats
}

impl RequestedShapesContainer {
    pub(super) fn build(shapes: Vec<Shape<LeafRequest>>, universe: LeafRequestSource, mode: &LoadMode) -> RequestedShapesContainer {
        RequestedShapesContainer {
            shapes: shapes,
    ...
    pub(crate) fn empty(mode: &LoadMode) -> RequestedShapesContainer {
        RequestedShapesContainer {
            shapes: vec![],
    ...
    pub(crate) fn merge(input: Vec<Arc<RequestedShapesContainer>>) -> RequestedShapesContainer {
    ...
    pub fn filter(&self, min_value: f64, max_value: f64) -> RequestedShapesContainer {
        RequestedShapesContainer {
