...
impl RectangleShape<Arc<dyn Transformer>> {
    fn demerge_by_variety(&self) -> Vec<(TransformerVariety,RectangleShape<Arc<dyn Transformer>>)> {
        let demerge = self.area.top_left().allotments().demerge(|x| {
            x.choose_variety()
        });
        let mut out = vec![];
