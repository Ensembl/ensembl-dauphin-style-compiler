...
impl SpaceBaseArea<f64,AnchoredLeaf> {
    pub fn spacebasearea_transform(&self, coord_system: &CoordinateSystem) -> SpaceBaseArea<f64,LeafStyle> {
...
impl SpaceBase<f64,AnchoredLeaf> {
    pub fn spacebase_transform(&self, coord_system: &CoordinateSystem) -> SpaceBase<f64,LeafStyle> {
        ...
        output.into_new_allotment(|x| { x.get_style().clone() })
    }
}
