...
use dauphin_interp::command::{ CommandInterpretSuite, CommandSetId, OpcodeMapping };
use dauphin_interp::util::DauphinError;
use dauphin_interp::util::cbor::{ cbor_array, cbor_int };
use serde_cbor::Value as CborValue;

pub struct OpcodeRemapper<'a> {
    suites: HashMap<(String,u32),CommandSetId>,
    mapping: &'a OpcodeMapping
}

impl<'a> OpcodeRemapper<'a> {
    pub fn new(mapping: &'a OpcodeMapping) -> OpcodeRemapper<'a> {
...
impl RemapperEndpoint {
    ...
    pub fn remap_program(&self, cis: &CommandInterpretSuite, remapper: &OpcodeRemapper, program: &CborValue) -> anyhow::Result<CborValue> {
        let mut out = vec![];
...
mod test{
    ...
    use dauphin_interp::command::{ Identifier, InterpLibRegister };
    use dauphin_interp::util::DauphinError;
    ...
    fn remapper_smoke() {
        ...
        /* remapper! (at last) */
        let mut remapper = OpcodeRemapper::new(&cis.opcode_mapper());
