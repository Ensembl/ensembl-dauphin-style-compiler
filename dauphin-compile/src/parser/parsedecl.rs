...
use super::node::{ ParserStatement, Statement };
...
pub(in super) fn parse_exprdecl(lexer: &mut Lexer, defstore: &DefStore) -> anyhow::Result<Vec<ParserStatement>> {
    lexer.get();
...
fn parse_args(lexer: &mut Lexer) -> anyhow::Result<Vec<IdentifierPattern>> {
    let mut out = vec![];
    get_other(lexer,"(")?;
    loop {
        ...
        if id.0.is_some() {
            return Err(parse_error("module illegal in argument specifier",lexer));
        }
        out.push(id);
        match lexer.get() {
            Token::Other(',') => (),
...
fn parse_block_statement(lexer: &mut Lexer, defstore: &DefStore) -> anyhow::Result<(Vec<Statement>,bool)> {
        ...
        Ok(mut stmts) => {
            ...
            for stmt in stmts.drain(..) {
                match stmt {
                    ...
                    _ => {
                        ffwd_error(lexer);
                        return Err(parse_error("Only regular statements allowed in macros",lexer));
                    }
                }            
            }
            Ok((out,eof_seen))
...
fn parse_block(lexer: &mut Lexer, defstore: &DefStore) -> anyhow::Result<Vec<Statement>> {
    let mut out = vec![];
    get_other(lexer,"{")?;
    loop {
...
pub(in super) fn parse_stmtdecl(lexer: &mut Lexer, defstore: &DefStore) -> anyhow::Result<Vec<ParserStatement>> {
    lexer.get();
    let identifier = parse_full_identifier(lexer,None)?;
    let args = parse_args(lexer)?;
...
pub(in super) fn parse_func(lexer: &mut Lexer, defstore: &DefStore) -> anyhow::Result<Vec<ParserStatement>> {
    lexer.get();
    ...
    if get_identifier(lexer)? != "becomes" {
        return Err(parse_error("missing 'becomes'",lexer));
    }
...
pub(in super) fn parse_proc(lexer: &mut Lexer, defstore: &DefStore) -> anyhow::Result<Vec<ParserStatement>> {
    lexer.get();
    let identifier = parse_full_identifier(lexer,None)?;
    let mut members = Vec::new();
    get_other(lexer,"(")?;
...
pub fn parse_signature(lexer: &mut Lexer, defstore: &DefStore) -> anyhow::Result<SignatureMemberConstraint> {
    let mut out = false;
    let mut stomp = false;
    loop {
...
fn id_to_type(pattern: &IdentifierPattern, lexer: &Lexer, defstore: &DefStore) -> anyhow::Result<BaseType> {
    let id = defstore.pattern_to_identifier(lexer,&pattern,true).map_err(|e| parse_error(&e.to_string(),lexer))?;
    if defstore.get_struct_id(&id.0).is_ok() {
        ...
        Err(parse_error(&format!("No such struct/enum '{}'",id.0),lexer))
    }
}

pub fn parse_type(lexer: &mut Lexer, defstore: &DefStore) -> anyhow::Result<MemberType> {
...
pub fn parse_typesig(lexer: &mut Lexer, defstore: &DefStore) -> anyhow::Result<ArgumentExpressionConstraint> {
    Ok(parse_typesigexpr(lexer,defstore)?)
}

pub fn parse_typesigexpr(lexer: &mut Lexer, defstore: &DefStore) -> anyhow::Result<ArgumentExpressionConstraint> {
    let pattern = parse_full_identifier(lexer,None)?;
    if pattern.0.is_none() {
        Ok(match &pattern.1[..] {
...
fn parse_struct_short(lexer: &mut Lexer, defstore: &DefStore) -> anyhow::Result<(Vec<MemberType>,Vec<String>)> {
...
fn parse_struct_enum_full(lexer: &mut Lexer, defstore: &DefStore) -> anyhow::Result<(Vec<MemberType>,Vec<String>)> {
    let mut types = Vec::new();
...
fn parse_struct_contents(lexer: &mut Lexer, defstore: &DefStore) -> anyhow::Result<(Vec<MemberType>,Vec<String>)> {
    let start = lexer.pos();
    Ok(match lexer.get() {
        Token::Identifier(_) => {
...
pub(in super) fn parse_struct(lexer: &mut Lexer, defstore: &DefStore) -> anyhow::Result<Vec<ParserStatement>> {
    lexer.get();
    let identifier = parse_full_identifier(lexer,None)?;
    get_other(lexer, "{")?;
...
pub(in super) fn parse_enum(lexer: &mut Lexer, defstore: &DefStore) -> anyhow::Result<Vec<ParserStatement>> {
    lexer.get();
    let identifier = parse_full_identifier(lexer,None)?;
    get_other(lexer, "{")?;
