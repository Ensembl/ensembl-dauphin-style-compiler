...
use super::arrayutil::{ 
    ...
    calculate_stretch_vertex_delta, calculate_stretch_vertex, apply_left, make_rect_elements
};
...
impl TapeGeometry {
    ...
    pub(crate) fn add_stretchtangle(&self, layer: &mut Layer, 
        ...
        let origins = interleave_line_x(&axx1,&axx2,hollow);
        let signs = vec![1.;len*if hollow {8} else {4}];
        elements.add(&self.variety.origins,origins)?; /* 4n */
        elements.add(&self.variety.vertexes,vertexes)?; /* 8n */
        elements.add(&self.variety.signs,signs)?; /* 4n */
        Ok(elements)
